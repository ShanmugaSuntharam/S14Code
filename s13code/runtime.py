        ...
        async def compose_surface(task: TaskSpec) -> dict[str, Any]:
            ...
            instruction = {
                ...
                "compose": ("Compose the RICHEST interface that serves the goal above, using ONLY catalog types, "
                            ...
                            "for /diff_text (the goal concerns a code change, fix, or patch) render a CodeDiff, "
                            "diff bound to /diff_text, title the literal /diff_file if present; "
                            "for /comments (the goal concerns reactions, feedback, or discussion) render a "
                            "CommentThread, comments bound to /comments; "
                            "for /locations (the goal concerns real places) render a Map, points bound to "
                            "/locations; "
                            "if the goal itself asks the user to APPROVE, REJECT, confirm, or decide on an action "
                            "(e.g. 'approve a remediation', 'confirm deployment') render an ApprovalCard: summary "
                            "bound to /summary or /intro, params bound to /summary, confirm action \"approve\", "
                            "reject action \"reject\" — this is required whenever the goal names an approval "
                            "decision, even alongside other components; "
