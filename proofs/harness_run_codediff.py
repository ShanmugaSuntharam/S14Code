...
that the new CodeDiff component enters a live Gemini-composed surface on its own.

Same harness as harness_run_map.py, but a single-goal, code-fix-flavored
prompt (no named entity list, so the run takes the compose_answer path
through the `content` role) that legitimately earns a `diff` field in the
content schema. The compose_surface prompt is NOT told about CodeDiff or
/diff_text: it only ever gets the catalog manifest and the available data
pointers, exactly like every other component. Whatever Gemini picks for
/diff_text here is its own choice.

Run it with S13Code's environment (it owns faiss / a2a deps):

    cd S14Code
    S13_GATEWAY_PROVIDER=gemini GLC_BASE_URL=http://127.0.0.1:8111 \
      uv run python proofs/harness_run_codediff.py

Writes proofs/harness_run_codediff.json.
"""
...
async def main() -> int:
    ...
    data_dir = Path(os.getenv("S13_DATA_DIR") or tempfile.mkdtemp(prefix="s13-harness-codediff-proof-"))
    ...
    print("=== HARNESS RUN SUMMARY (CodeDiff proof) ===")
    ...
    print(f"diff_text in dm  : {(surface_result.get('data_model') or {}).get('diff_text')!r}")
    print(f"codediff used?   : {'CodeDiff' in (validator.get('component_types') or [])}")
