...
that the new Map component enters a live Gemini-composed surface on its own.

Same harness as harness_run.py, but a single-goal geographic prompt (no named
entity list, so the run takes the compose_answer path through the `content`
role) that legitimately earns a `locations` field in the content schema. The
compose_surface prompt is NOT told about Map or /locations: it only ever gets
the catalog manifest and the available data pointers, exactly like every other
component. Whatever Gemini picks for /locations here is its own choice.

Run it with S13Code's environment (it owns faiss / a2a deps):

    cd S14Code
    S13_GATEWAY_PROVIDER=gemini GLC_BASE_URL=http://127.0.0.1:8111 \
      uv run python proofs/harness_run_map.py

Writes proofs/harness_run_map.json.
"""
...
OUT = Path(__file__).parent / "harness_run_map.json"
TASK = ("Build a travel-planning interface for European landmarks worth visiting, "
        "with their real-world locations.")
...
async def main() -> int:
    ...
    data_dir = Path(os.getenv("S13_DATA_DIR") or tempfile.mkdtemp(prefix="s13-harness-map-proof-"))
    ...
    result = await runtime.run(
        prompt=TASK,
        scope=MemoryScope("s14-proof", "harness-map", "composer", "s13code"),
        llm=lambda prompt, system: gateway.complete(prompt, system),
        source_uri="proof://harness/compose_surface_map",
    ...
    print("=== HARNESS RUN SUMMARY (Map proof) ===")
    ...
    print(f"locations in dm  : {(surface_result.get('data_model') or {}).get('locations')}")
    print(f"map used?        : {'Map' in (validator.get('component_types') or [])}")
